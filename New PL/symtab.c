...
st_node_t *symtab_insert(symtable *st, char *name, symtab_type class,
  ...
  /* If entry is for a function, allocate some space for the argument type list
     that interpreter.y will assign later. This assignment can not happen yet
...
/* 이 부분이 필요한지 의문??
Creates the input/output entries in the symbol table for read/write procedures.
*/
...
/* Hack.   
심볼 테이블의 변수 클래스 항목 수와 함수 / 프로 시저 클래스 항목의 인수 수를 계산하여 오프셋을 계산
If the procedure is "write", then add three (2 for the format
